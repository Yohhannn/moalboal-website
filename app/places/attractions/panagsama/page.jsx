...
const AttractionInfo = () => {
  const images = [
    ...
    "/attractions_assets/panagsama/panagsamaATT4.jpg",
    "/attractions_assets/panagsama/panagsamaATT5.jpg",
    "/attractions_assets/panagsama/panagsamaATT6.jpg",
    "/attractions_assets/panagsama/panagsamaATT7.jpg",
  ];

  const reviews = [
    { name: "Lukas ", text: "Even though most of the coral was destroyed during a recent typhoon .. Panagsama Beach is still a great diving spot to experience the fantastic sardines runs & the sea turtles .. all this is just meters off the beach. There are plenty of dive companies at very reasonable prices to help you out with your dive .. these dive companies can even GoPro your experiences", date: "Nov 30, 2024", rating: 4.5 },
    { name: "Eloise ", text: "We had an amazing time seeing the Sardines run and turtles at Panagsama beach! We paid £7 per person for a private tour with the company 'Friends Adventure in Panagsama' and it was soooo worth it! The price included a guide, life jackets, snorkelling equipment and safe locker storage for our bags. Our guide was absolutely brilliant! He took a float for us to hold on to while we looked down and snorkelled with the sardines, turtles and other fish. He also took our underwater camera and got some amazing photos and videos for us. I honestly can't recommend this enough", date: "Dec 1, 2024", rating: 4 },
    { name: "Mary ", text: "You don't really need to swim far because the sardines and turtles are already visible. I stayed at the beach for almost 5 hours, watching the sardines and turtles, rest and sunbathe a little and watch the sardines again because I couldn't get enough of it 😁😁😁", date: "Dec 2, 2024", rating: 5 },
  ...
  {rows.map((row, rowIndex) => (
    <div key={rowIndex} className={`flex flex-wrap justify-center gap-6 ${rowIndex > 0 ? "mt-6" : ""}`}>
      {row.map((attraction, index) => (
        <div
          key={index}
          className={`relative rounded-t-3xl shadow-xl w-full sm:w-[48%] md:w-[50%] lg:w-[30%] min-w-[300px] min-h-[400px] flex flex-col bg-gray-900
